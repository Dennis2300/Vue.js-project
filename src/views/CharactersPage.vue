<template>
    <div class="p-4">
      <h1 class="text-2xl font-bold mb-4">Genshin Impact Characters</h1>
      ...
      <ul v-if="!loading && !error && characters?.length" class="space-y-2">
        ...
        >
          ...
          <span class="text-yellow-500">{{ character.rarity?.replace('_', ' ') || 'Unknown' }}</span>
          <a :href="character.wiki_url" target="_blank" class="text-blue-500 hover:underline">Wiki</a>
        </li>
      </ul>
  
      <!-- Show message if no characters are found -->
      <p v-if="!loading && !error && !characters?.length" class="text-gray-500">No characters available.</p>
    </div>
  </template>
